...
function App() {
  ...
  useEffect(() => {
    axios
      .get("https://autoconsult-backend-3.onrender.com/api/services/")
      .then((res) => setServices(res.data));
    axios
      .get("https://autoconsult-backend-3.onrender.com/api/projects/")
      .then((res) => setProjects(res.data));
    axios
      .get("https://autoconsult-backend-3.onrender.com/api/requests/")
  ...
  const deleteRequest = (id) => {
    if (window.confirm("Are you sure you want to delete this inquiry?")) {
      axios
        .delete(
          `https://autoconsult-backend-3.onrender.com/api/requests/${id}/`,
        )
        .then(() => {
  ...
  const exportToExcel = () => {
    ...
    const rows = processedProjects.map(
      (p) => `${p.client_name},${p.title},${p.status}\n`,
    );
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .post(
        "https://autoconsult-backend-3.onrender.com/api/requests/",
        formData,
      )
      .then((res) => {
  ...
  const addService = (e) => {
    ...
    axios
      .post(
        "https://autoconsult-backend-3.onrender.com/api/services/",
        newService,
      )
      .then((res) => {
  ...
  const styles = {
    ...
    blob2: {
      ...
      borderRadius: "50%",
      background:
        "radial-gradient(circle, rgba(244,114,182,0.08) 0%, rgba(255,255,255,0) 70%)",
      bottom: "-100px",
      left: "-150px",
      zIndex: 0,
      filter: "blur(60px)",
    ...
    return (
      <div style={styles.wrapper}>
        ...
        <div style={{ ...styles.container, padding: "20px 15px" }}>
              ...
              <div style={{ marginBottom: "20px" }}>
                ...
                >
                  <div
                    style={{
                      ...styles.graphBar,
                      width: `${Math.min(projects.length * 10, 100)}%`,
                    }}
                  ></div>
                </div>
              </div>
              <div>
                ...
                >
                  <div
                    style={{
                      ...styles.graphBar,
                      width: `${Math.min(services.length * 10, 100)}%`,
                      backgroundColor: "#10b981",
                    }}
                  ></div>
                </div>
          ...
          >
            {/* Mobile Scroll Wrapper - Is se table card ke andar hi rahegi */}
            <div
              style={{
                width: "100%",
                overflowX: "auto",
              ...
              >
                <thead style={{ backgroundColor: "#f8fafc" }}>
                  <tr style={{ textAlign: "left" }}>
                    <th style={{ padding: "12px", fontSize: "14px" }}>
                      Client Info
                    </th>
                    <th style={{ padding: "12px", fontSize: "14px" }}>
                      Service
  ...
  return (
    <div style={styles.wrapper}>
      ...
      <img
        src="https://pngimg.com/uploads/car/car_PNG7544.png"
        style={styles.carShadowLeft}
        alt=""
      />
      <img
        src="https://pngimg.com/uploads/car/car_PNG7544.png"
        style={styles.carShadowRight}
        alt=""
      />

      <nav style={styles.nav}>
        <h1 style={{ fontSize: "24px", fontWeight: "bold" }}>
          AUTO<span style={{ color: "#60a5fa" }}>CONSULT</span>
        </h1>
        <button
      ...
      >
        ...
        >
          At <strong>AutoConsult</strong>, we bridge the gap between complex
          automotive challenges and smart strategic solutions. We empower
          businesses to make informed decisions through premium consulting and
      ...
      {showLoginModal && (
        <div style={styles.modalOverlay}>
          <div style={styles.modalCard}>
            ...
            <h2
              style={{ textAlign: "center", marginTop: 0, fontWeight: "300" }}
            >
              Staff Access
            </h2>
      ...
      <div style={styles.container}>
        <div style={styles.statsGrid}>
          <div style={styles.statCard}>
            <h3 style={{ fontSize: "32px", margin: "0" }}>{projects.length}</h3>
            <p style={{ color: "#64748b", margin: "5px 0 0 0" }}>
              Active Projects
            </p>
          </div>
          <div style={styles.statCard}>
            <h3 style={{ fontSize: "32px", margin: "0" }}>{services.length}</h3>
            <p style={{ color: "#64748b", margin: "5px 0 0 0" }}>
              Premium Services
            </p>
          </div>
        ...
        >
          {filteredServices.map((s) => (
            <div
              key={s.id}
              style={{ ...styles.card, borderTop: "6px solid #2563eb" }}
            >
        ...
        <div style={styles.estimator}>
          ...
          <p style={{ marginBottom: "30px", opacity: 0.9 }}>
            Get an instant high-level quote for your strategy
          </p>
          <select
          ...
          >
            ...
            {services.map((s) => (
              <option key={s.id} value={s.base_price}>
                {s.name}
              </option>
            ))}
          </select>
        ...
        >
          ...
          >
            ...
            <input
              ...
              onChange={(e) =>
                setFormData({ ...formData, service_interested: e.target.value })
              }
            />
            <textarea
            ...
            />
            <button type="submit" style={{ ...styles.btn, padding: "15px" }}>
              Submit Request
            </button>
          </form>
        </div>
      ...
      <footer style={styles.footer}>
        <p>© 2026 AutoConsult Strategy Portal.</p>
      </footer>
    </div>
  );
}
